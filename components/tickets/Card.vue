    ...
    <div class="overflow-y-scroll h-96 mt-4">
      <table class="table w-full z-10">
        ...
        <tr class="bg-base-300 border-gray-500 z-10">
          <th class="border border-gray-500 w-36">Id</th>
          <th class="border border-gray-500 w-36">Datum</th>
          ...
          <th v-else class="border border-gray-500 w-40">Geschlossen am</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="ticket in ticketData" class="bg-white cursor-pointer hover:bg-primary-content" @click="openTicket(ticket.id)">
          <td class="border border-gray-500">{{ticket.id}}</td>
          <td class="border border-gray-500">{{ ticket.createDate }}</td>
          ...
          <td v-else class="border border-gray-500">{{ ticket.closeDate }}</td>
        </tr>
        </tbody>
      </table>
    </div>
